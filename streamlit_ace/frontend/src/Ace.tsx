...
const Ace = ({ args, theme }: AceProps) => {
  ...
  const createCustomCompleter = (completers: string[]) => {
    return {
      getCompletions: (
        editor: IAceEditor,
        session: any,
        ...
        const customCompletions = completers.map((word) => ({
          value: word,
          meta: "custom",
        }));
  ...
  useEffect(() => {
    if (editorRef.current && args.customCompleters) {
      const editor = editorRef.current.editor;
      const customCompleter = createCustomCompleter(args.customCompleters);
      const langTools = ace.require("ace/ext/language_tools");
      const existingCompleters = langTools.getCompleters();
      langTools.setCompleters([...existingCompleters, customCompleter]);
      editor.setOptions({
        enableBasicAutocompletion: true,
        enableLiveAutocompletion: true,
      });
    }
  }, [args.customCompleters]);
